...
def showCurveAndPhi(I, phi, color):
    ...
    plt.hold(True)
    # CS = plt.contour(phi[:, :, I.shape[2] / 2], colors=color)
    plt.hold(False)
...
def sussman(D, dt):
    ...
    f_p = f
    f_n = f.copy()
    a_p[np.where(a_p < 0)] = 0
    a_n[np.where(a_n > 0)] = 0
    b_p[np.where(b_p < 0)] = 0
    b_n[np.where(b_n > 0)] = 0
    c_p[np.where(c_p < 0)] = 0
    c_n[np.where(c_n > 0)] = 0
    d_p[np.where(d_p < 0)] = 0
    d_n[np.where(d_n > 0)] = 0
    e_p[np.where(e_p < 0)] = 0
    e_n[np.where(e_n > 0)] = 0
    f_p[np.where(f_p < 0)] = 0
    f_n[np.where(f_n > 0)] = 0
    # if (a_p[i].any() < 0):
    #     a_p[i] = 0
    # if (a_n[i].any() > 0):
    #     a_n[i] = 0
...
def shiftD(M):
    shift = M[:, list(range(1, M.shape[1])) + [M.shape[1] - 1], :]
    return shift


def shiftL(M):
    shift = M[:, :, list(range(1, M.shape[2])) + [M.shape[2] - 1]]
    return shift


def shiftR(M):
    shift = M[:, :, [0] + list(range(0, M.shape[2] - 1))]
    return shift


def shiftU(M):
    shift = M[:, [0] + list(range(0, M.shape[1] - 1)), :]
    return shift


def shiftF(M):
    shift = M[[0] + list(range(0, M.shape[0] - 1)), :, :]
    return shift


def shiftB(M):
    shift = M[list(range(1, M.shape[0])) + [M.shape[0] - 1], :, :]
    return shift
...
if __name__ == "__main__":
    # im_path = 'C:\\Users\\Keren Meron\\Dropbox\\Cool Engineering\\Final Project\\data\\mri_good_quality'
    # im_path = 'C:\\Users\\Eldan\\Dropbox\\\אוניברסיטה\\תקייה משותפת -פרוייקט גמר\\engineer\\Final ' \
    #           'Project\\data\\brain_for_test.png'
    # img = nd.imread(im_path, flatten=True)
    import nibabel as nib
    new_dir = 'C:\\Users\\Eldan\\Dropbox\\University\\Final  Project - joint dir\\engineer\\Final ' \
              'Project\\data\\Brain MRI\\test'
    img = nib.load(new_dir+'\\5_fetal.nii.gz')

    data = np.transpose(img.get_data(), (1, 2, 0))
    print(data.shape)
    mask = np.zeros(data.shape)
    mask[ 230:260,230:260,2:8] = 1
    # image_clor = img[188,125]
    # img[np.where(img<=image_clor+25)] = 0
    # plt.imshow(img, cmap = 'gray')
    # plt.show()
    a,b,c = chanvese3d(data, mask, max_its=1000, display=False, alpha=0.1)
    print(c)
    print(a.shape)
    fig = plt.figure()
