...
def chanvese(I, init_mask, max_its=1000, alpha=0.2,
             thresh=0.5, color='r', display=False):
    I = I.astype(np.float)
...
if __name__ == "__main__":
    img = nd.imread('image35.png', flatten=True)
    mask = np.zeros(img.shape)
    mask[ 185:188,115:125] = 1
    # image_clor = img[188,125]
    # img[np.where(img<=image_clor+25)] = 0
    plt.imshow(img, cmap = 'gray')
    plt.show()
    chanvese(img, mask, max_its=5000, display=True, alpha=0.1)